 ...
 * Changes in the inputs are temporarily stored in a map.
 * When the refresh rate elapses the new output and validity is generated 
 * from the inputs and a new object created.
 * 
 ...
 * @param actualInputs: The list of monitor points in puts that generated the actual output
...
class AlarmSystemComponent[T] (
  ...
  def shelve(newShelveState: Boolean): AlarmSystemComponent[T] = {
    ...
    new AlarmSystemComponent[T](ident,shelvedTypedMP,requiredInputs,actualInputs,script)
  ...
  def ack(): AlarmSystemComponent[T] = {
    ...
    new AlarmSystemComponent[T](ident,ackedTypedMP,requiredInputs,actualInputs,script)
  }
}
